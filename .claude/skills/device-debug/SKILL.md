...
- この行は **RGB への変換とフレームバッファへの格納まで通った**ときに出る。**「届いていない」と「届いたが捨てた」は区別が要る**
  - この行も下の `warn` も無い → コールバックまで 1 枚も来ていない。信号なし、またはデバイス側が出していない
  - この行が無く `warn` がある → フレームは来ているが捨てている。`warn` の内容（短いフレーム / デコード失敗 / ロック取得失敗）が理由
- ストリームを開いてから 1 枚目までは正常時でも 0.81 秒かかる。数秒待っても出なければ異常
- 2 枚目以降は `trace` の `フレームが届いた` に落としてある。毎フレーム `info` を出さないため
- 捨てた場合の `warn` はいずれも**初回だけ**出る。フォーマットが YUYV でない・幅が奇数なら `YUY2 の高速パスを使えないのでデコーダへフォールバックする`、YUY2 だがデータが足りなければ `YUY2 のフレームが短いので破棄した`、デコーダが失敗すれば `フレームのデコードに失敗した`、フレームバッファのロックを取れなければ `フレームバッファのロックを取得できないのでフレームを捨てた`
