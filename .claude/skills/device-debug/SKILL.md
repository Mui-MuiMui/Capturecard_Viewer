...
- **呼び出し側は失敗を握り潰す**（`if let Ok(caps) = ...`）。UI には何も出ず、フォーマットや解像度の選択肢が空のままになる。**失敗の理由は `video.rs` 側が `warn` の `デバイス能力を取得できない（N ms）: <理由>` で残す**ので、ログを見れば分かる
- `get_device_capabilities` は `Camera::new` で**キャプチャ中のデバイスをもう一度開く**。UI スレッドから同期で呼ぶため、時間がかかると設定ダイアログがその場で固まる。所要時間は `info` の `デバイス能力を取得した（<デバイス名>、N ms、YUY2: n 件、…）` に出る
