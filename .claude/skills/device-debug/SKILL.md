...
**`trace` は原則使わない。** eframe / winit のイベントが毎フレーム流れ、12 秒の起動で 9,000 行・1.1MB に達する（実測）。自前のログはその中に数十行しかない。デバイス調査で `trace` にして増えるのは 2 秒ごとの再適用の記録だけで、得るものがない。
...
この状態はログから見えない。理由は 2 つあり、どちらも既知の穴。

- **`src/video.rs` には `log` の呼び出しが 1 つもない**（他モジュールは 5〜40 箇所）。フレームの到着も変換の失敗もどこにも残らない
- **nokhwa のフレーム取得ループがエラーを捨てている。** `nokhwa-0.10.11/src/threaded.rs` の `camera_frame_thread_loop` が `if let Ok(frame) = camera.frame()` で握り潰し、sleep も挟まずに回り続ける。**CPU 使用率が高いのに映像が出ない場合はこれ**

切り分けるには `FrameBuffer::generation` が進んでいるかを見るしかない。調査のために一時的に `video.rs` へ `trace!` を足す場合は、**毎フレーム出すログを残したままコミットしない**（1080p60 で毎秒 60 行）。
...
`src/ui.rs` がビデオデバイスを変えたときに `VideoCapture::get_device_capabilities` を呼ぶ。

- **失敗しても握り潰される**（`if let Ok(caps) = ...`）。ログにも UI にも何も出ず、フォーマットや解像度の選択肢が空のままになる
- `get_device_capabilities` は `Camera::new` で**キャプチャ中のデバイスをもう一度開く**。UI スレッドから同期で呼ぶため、時間がかかると設定ダイアログがその場で固まる
- 全フォーマットで `compatible_list_by_resolution` が失敗したときは、コード内にベタ書きされた既定の解像度リストが返る。**画面に出ている選択肢がデバイスの実際の能力とは限らない**
...
- 実測では MediaFoundation の列挙は 3ms、映像の接続全体でも 28〜91ms。**待つべき時間として 2 秒は過大**
...
いずれも `映像デバイスへの接続に失敗した（1 回目）` の理由を記録する。`Device '<名前>' not found` なら列挙に間に合っていない（= 待てば解決する）、`Failed to create camera` なら列挙には出るが開けない（= 待っても解決しない可能性がある）。**この区別がリトライ設計を決めるので、理由の文字列を必ず残す。**
