...
| `Camera::new` | `映像ストリームを開いた` の中の `Camera::new` | 19.1〜76.5ms（下記参照） |
...
| 音声の接続 | `音声デバイスへの接続を試す（1 回目）` → `音声デバイスに接続した` | 454〜571ms |
| プロセス開始 → 接続完了 | `を起動した` → `音声デバイスに接続した` | **0.70〜0.87 秒** |
| ストリーム開始 → 1 枚目 | `映像ストリームを開いた` → `最初のフレームが届いた` | 812〜814ms |
| プロセス開始 → 映像が出る | `を起動した` → `最初のフレームが届いた` | **1.06〜1.12 秒** |
...
| 音声のフォールバック | **しない。** 設定のデバイスが戻るまで同じ対象で試し続ける。設定が「既定のデバイス」（入出力とも未指定）のときだけ、3 回目の失敗の直後にレートとチャンネル数を `None` にして 1 度試す |
...
- `設定の音声デバイスに 3 回続けて接続できない。既定のデバイスへは倒さず、戻るまで再試行を続ける`（`warn`）。**この行が出ていたら、音声は設定のデバイスを待っている状態。** マイクなど別のデバイスへは倒していない（#134）
- `既定のデバイスで 3 回続けて失敗したので、レートとチャンネル数をデバイス任せにして試す` → `既定のデバイスで音声に接続した（レートとチャンネル数はデバイス任せ）`（`info`）。**設定が「既定のデバイス」のときだけ出る。** 開く相手は変わらない
