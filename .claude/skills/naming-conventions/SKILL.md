...
**重要な結論は履歴ではなく `CLAUDE.md` / `GUARDRAIL.md` / `docs/design/` に書く。**

新しいセッションで最初に読まれるのは `CLAUDE.md`（と `@GUARDRAIL.md` で取り込まれる `GUARDRAIL.md`）とコードであって、`git log` ではない。履歴に書いた判断は、明示的に調べない限り参照されない。
