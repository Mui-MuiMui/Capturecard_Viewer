...
| ボタン | 動作 |
|---|---|
| 適用 | 編集内容を動作中のアプリへ反映します。設定ファイルへは書き込まず、画面も閉じません |
| OK | 反映したうえで設定ファイルへ保存し、画面を閉じます |
| キャンセル | まだ反映していない編集を破棄して画面を閉じます。**「適用」で反映済みの内容は元に戻りません** |
| ×（タイトルバー） | キャンセルと同じです |

- 「適用」で反映した内容は動作中の設定の一部になるため、そのあとウィンドウの移動や音量変更をきっかけに設定ファイルへ書き出されます。ファイルに残したくない場合は「適用」を押さないでください。
