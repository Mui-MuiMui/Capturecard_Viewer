...
- スクリーンショットの保存スレッド（撮影ごとに spawn。JPEG / PNG エンコードとファイル書き出し、設定によってはクリップボードへの転送を行う）
...
- `take_screenshot` は settings → video → screenshot の順に 1 つずつ取り、エンコードと保存とクリップボードへの転送は別スレッドへ渡す
