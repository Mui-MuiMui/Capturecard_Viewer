...
例外は、**設定が「既定のデバイス」（入力・出力とも未指定）の場合だけ。** このときは 3 回目の失敗のあとにサンプリングレートとチャンネル数をデバイス任せにして 1 度試す（`AudioFallbackAction::RelaxFormat`）。開く相手は変わらないので、意図しないデバイスを掴むことはない。
...
- 設定のデバイスに繋がらないまま再試行を続けることは、3 回目に 1 度だけ `warn` で残す。毎回出すとログが埋まり、一度も出さないと「音が出ない」の調査でこの状態に気付けない
...
| 音声のフォールバック | **しない。** 設定が「既定のデバイス」のときだけ、3 回目の失敗の直後にレートとチャンネル数を緩めて 1 度試す（「音声は繋がらなくても別のデバイスへ倒さない」を参照） |
