...
`docs/ARCHITECTURE.md` の設計の前提（低遅延 > 単体で動く > 原因が追える > 画質）に沿って判断する。

- **依存は増やさない。** 標準ライブラリで書ける処理のために依存を足さない
- **更新は目的を持って行う。** 「新しいから」ではなく、必要な機能、不具合の修正、セキュリティのいずれかを理由にする
- **メジャー更新は単独の PR にする。** 特に egui は破壊的変更が広範囲に及ぶ
- `Cargo.lock` をコミットして、ビルドの再現性を確保する（未対応。バックログにあり）

0.x のクレートは**マイナー更新が破壊的変更**である点に注意する。この表の「差」はその前提で読む。
...
**先に `Cargo.lock` をコミットしてから着手すること。** 現在はビルドのたびに依存の解決結果が変わりうるため、更新の前後で何が変わったのか追跡できない。
