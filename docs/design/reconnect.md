...
- **DirectShow のデバイスは、グラフのイベントでも切断を検出する（#229）。** DirectShow のフィルターグラフはデバイスが消えると `EC_DEVICE_LOST` を積む。`CaptureGraph::poll_device_lost` がこれを `IMediaEventEx::GetEvent`（タイムアウト 0、待たない）で読み、`VideoLinkState::device_lost` を立てる。`decide_video_link` は `device_lost` なら途絶時間を待たずに切断とみなすので、抜いた次の監視の周期（100〜500ms）で反応する
  ...
  - 切断とみなすイベントは `is_device_lost_event` が決める。`EC_DEVICE_LOST` は `lparam2` が 0（取り外し）のときだけで、1（再び使えるようになった）は切断ではない。エラーでグラフやストリームが止まった `EC_ERRORABORT` / `EC_ERRORABORTEX` / `EC_STREAM_ERROR_STOPPED` も、そのままではフレームが戻らないので同じ扱いにする。グラフを動かす指示が失敗した `EC_ERROR_STILLPLAYING`（1 枚目の前にグラフが止まりうる知らせ）も含める（#232）。開いた直後に毎回これを出すデバイスでも開き直しが待ち無しで回らないのは、`ConnectRetry` の「成功から次の開き直しまでの下限」（後述）が抑えるため読んだイベントは関係ないものも含めて `FreeEventParams` で解放する
...
| 成功から次の開き直しまで | 最低 1 秒（`RECONNECT_MIN_INTERVAL_AFTER_SUCCESS`）。この間に要求が来ても積むだけで開かない |
...
- **バックオフとは別の軸。** `is_due` は「失敗のバックオフの期限」と「成功からの下限」の両方を満たしたときだけ真になる。下限は固定で、繰り返しても伸びない。成功と切断を繰り返す限り、開き直しは 1 秒に 1 回で頭打ちになる
- **1 秒の根拠。** 開いてから 1 枚目が届くまでが実測 0.8 秒で、それより短い間隔で開き直しても映像が出る前に閉じるだけになる。長くすると、開いた直後に本当に抜かれたときの復帰がその分だけ遅れる
- **`request_now()` でも下限は捨てない。** 切断の検出（映像の途絶・`device_lost`、音声のストリームエラー、既定デバイスの切り替え）はどれも `request_now()` で要求を積むので、ここで捨てると下限が効かない。右クリックの「デバイス再接続」や設定の変更も同じく下限を受けるが、待つのは成功から最大 1 秒なので経路で分けていない
- **音声にも効く。** 音声も同じ `ConnectRetry` を使う。音声のストリームエラーには別に `AUDIO_ERROR_RECONNECT_MIN_INTERVAL`（5 秒）があるが、これは「前回エラーで開き直しを要求してから」の間隔で、エラー以外の経路（既定デバイスの切り替え、映像の復帰に合わせた開き直し）には掛からない。成功からの下限はそれらも含めて一律に掛かる
- **`thread::sleep` は使わない。** 時刻を覚えて `tick` のたびに比べるだけで、上の「接続を待つために `thread::sleep` を使わない」はそのまま守る
