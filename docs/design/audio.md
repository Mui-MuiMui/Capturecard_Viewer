...
リングバッファの長さ（`settings::AudioSettings::audio_buffer_ms`、20〜200ms、既定 50ms）がそのまま音声の遅延になる。短くすると遅延は減るが、出力コールバックが次のぶんを取りに来たときに入力が間に合わずアンダーラン（プチプチという音）が出る。**どこが最適かは PC の負荷とデバイス次第で決め打ちできない**ので、設定ダイアログのスライダーで選ばせる。
