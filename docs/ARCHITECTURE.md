...
**待ち時間を `thread::sleep` で作らない。** 再試行の間隔やタイムアウトは「次に試してよい時刻」として持ち、`update()` で期限を見て進める。眠らせるとその間フレームが進まず、ウィンドウが応答しなくなる。デバイスの接続はこの形になっている（`ConnectRetry`）。デバイスを開く処理自体はまだ UI スレッド上だが、1 回の試行は数百 ms で返るため、`sleep` で秒単位止まることは無くなった。
