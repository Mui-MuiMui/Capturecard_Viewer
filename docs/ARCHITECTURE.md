...
| UI は状態を持たない | `ui.rs` から `static` / `static mut` は消え、ダイアログの状態はすべて `SettingsDialogState`（`CaptureCardViewer` が所有）にある。ただしホットキー入力ダイアログは `&mut` で受けた値を直接書き換える | UI 層をイベント返却型にする |
