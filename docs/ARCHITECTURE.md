...
| UI をブロックしない | デバイス能力の取得もスクリーンショットの JPEG エンコードも別スレッドへ移した。デバイスの接続と再試行の `sleep` は UI スレッド上のまま | デバイスアクセスをワーカースレッド + チャネルにする |
...
| 統計の公開 | 映像側は `VideoCapture::stats()` で読み出せ、右クリックの「情報表示」で OSD に出る。音声のアンダーラン回数はまだ収集していない | 映像側は完了。音声側はタスク未登録 |
