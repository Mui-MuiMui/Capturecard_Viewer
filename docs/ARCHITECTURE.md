...
**ここは `src/status.rs` として実装済み**（ホットキーの登録失敗を除く。記録の仕組みはあり、登録処理側から呼べば出る）。発生源ごとに直近の 1 件だけを `ErrorCenter` が持ち、新しい失敗は `TransientOverlay` に数秒出す。同じ発生源で同じ文言が続く間は間引く。接続の再試行は最大 5 秒間隔で無限に続くため、間引かないと通知が出っぱなしになるため。
