...
| フレームを複製しない | 毎フレーム全画素を `clone` | get_latest_frame の全画素コピー削減 |
| 新フレームの判別 | 新しいフレームがなくても前回の画を返す | デバイス切断検出と自動再接続 |
...
| 設定を壊さない | `#[serde(default)]` なし、失敗時は黙って初期化 | 設定構造体に serde(default) がない |
