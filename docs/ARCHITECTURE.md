...
モジュールごとにエラー型を定義し、呼び出し側が種別で分岐できる状態にする。**ここは `VideoError` / `AudioError` / `ScreenshotError` / `HotkeyError` / `SettingsError` として実装済み**（それぞれ `src/video.rs` / `src/audio/mod.rs` / `src/screenshot.rs` / `src/hotkey.rs` / `src/settings.rs`）。
