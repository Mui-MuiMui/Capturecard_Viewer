...
モジュールごとにエラー型を定義し、呼び出し側が種別で分岐できる状態にする。**ここは `VideoError` / `AudioError` / `ScreenshotError` / `HotkeyError` / `SettingsError` として実装済み**（それぞれ `src/video/mod.rs` / `src/audio/mod.rs` / `src/screenshot.rs` / `src/hotkey.rs` / `src/settings.rs`）。
...
**表示用の日本語は、その型の `Display` として各モジュールに置く。** 以前はここを「UI 層で組み立てる」としていたが、エラーの中身と文言が離れているとバリアントを増やすたびに `status.rs` 側の `match` を足す必要があり、片方の更新漏れで英語の文言が残っていた。文言は中身のすぐ隣に置き、`status.rs` は定型文（`ErrorSource::headline`）との連結と表示用の組み立てだけを持つ。
...
| trait による抽象化 | デバイスワーカーは `app::backend` の `VideoBackend` / `AudioBackend` 越しにしかデバイスへ触らない。本番の実装は `VideoCapture` / `AudioCapture`、テストはモック。**コールバックの経路には挟んでいない**（映像フレームと音量は共有ハンドルを直に流れる）。実機なしで動くフェイクデバイスは未実装 | テスト用のフェイクデバイスをアプリ内バックエンドとして用意する |
