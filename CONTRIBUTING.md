...
| [AGENTS.md](AGENTS.md) | Claude Code 以外の AI エージェント向けの入口。道案内だけ |
...
- **ユーザーから見える変更だけを書きます。** 挙動が変わらない内部のリファクタリングや、既存の文書の文言の修正は書きません
- **読み手から見て増えるもの（新しいドキュメント）は書きます。** 「未リリース」に `CLAUDE.md` や `docs/*` が並んでいるのはこのためです
...
このリポジトリは [Claude Code](https://claude.com/claude-code) を使って開発しています。リポジトリの構造と作業の進め方は `CLAUDE.md` に、守るべき決まりは `GUARDRAIL.md` に、設計判断の理由は `docs/design/` に、検証・命名・テスト・リリースの各手順は `.claude/` 以下にまとめてあります。 Claude Code 以外の AI エージェント（AGENTS.md を読むツール）向けの入口は `AGENTS.md` です。
